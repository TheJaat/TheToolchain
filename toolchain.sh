...
# Build and install GCC (Bootstrap) with multilib support
mkdir -p "$BUILD_DIR/gcc"
cd "$BUILD_DIR/gcc" || exit 1
"$SRC_DIR/gcc-$GCC_VERSION/configure" --target=$TARGET --prefix="$PREFIX" --disable-nls --enable-languages=c,c++ --without-headers --enable-multilib
make all-gcc
make install-gcc
make all-target-libgcc
make install-target-libgcc

echo "Cross-compiler for $TARGET (multilib enabled) installed in $PREFIX"
